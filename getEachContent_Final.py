...
csv=pd.read_csv('<insert path to CSV file>', encoding = "ISO-8859-1")
...
for href in hrefs:
    if href == 'exception error':
        print('this one was broken when we scraped it')
    else:
        try:
            articleContent = getArticleContent(href)
            filename = href.replace("https://www.coindesk.com/", "")
            articleContentFileName = "C:/Users/Nick/Desktop/data_foundations/tron_content/" + filename.replace("/",
                                                                                                               "") + ".txt"
            file = open(articleContentFileName, 'w')
            file.write(articleContent)
            print(str(iternum) + ' / ' + str(num_links))
        except:
            num_broken_links += 1
            print(str(iternum) + ' / ' + str(num_links))
            print('No. Broken Links: ' + str(num_broken_links))